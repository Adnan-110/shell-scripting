...
echo -n "Downloading the Cart Component :"
curl -s -L -o /tmp/${COMPONENT}.zip $COMPONENT_URL      &>> $LOG_FILE
stat $?

echo -n "Performing Clean-Up of the Cart Component :"
rm -rf $APPUSER_HOME    &>> $LOG_FILE
stat $?

echo -n "Extracting the Cart in roboshop user :"
cd /home/roboshop
unzip -o /tmp/${COMPONENT}.zip      &>> $LOG_FILE
stat $?

echo -n "Configuring the Cart Component Permissions :"
mv ${APPUSER_HOME}-main $APPUSER_HOME &>> $LOG_FILE
chown -R $APPUSER:$APPUSER $APPUSER_HOME 
chmod -R 770 $APPUSER_HOME
stat $?

echo -n "Installing the Cart Component Dependencies :"
cd $APPUSER_HOME
npm install      &>> $LOG_FILE
stat $?

echo -n "Configuring the Cart Component Systemd File :"
sed -i -e 's/REDIS_ENDPOINT/redis.roboshop.internal.com/' -e 's/CATALOGUE_ENDPOINT/catalogue.roboshop.internal.com/' $APPUSER_HOME/systemd.service $APPUSER_HOME/systemd.service 
mv ${APPUSER_HOME}/systemd.service /etc/systemd/system/${COMPONENT}.service
stat $?

echo -n "Starting the Cart Component Services :"
systemctl daemon-reload         &>> $LOG_FILE
systemctl enable $COMPONENT     &>> $LOG_FILE
systemctl restart $COMPONENT    &>> $LOG_FILE
stat $?

echo -e "************\e[33m Configuration of the Cart Component is Completed \e[0m************"
